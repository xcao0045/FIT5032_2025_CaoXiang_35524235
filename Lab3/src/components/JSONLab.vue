...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="author in authors" :key="author.id" :class="{ highlight: isHighlighted(author.name) }">
      ...
      <!-- TODO: CODE TO RENDER LIST OF AUTHORS HERE -->
      <ul>
        <li v-for="author in modernAuthors" :key="author.id" :class="{ highlight: isHighlighted(author.name) }"> 
          {{ author.name }} ({{ author.birthYear }})
        </li>
      </ul>
      ...
      <p>Finding by property: 
        <span :class="{ highlight: isHighlighted(orwell?.name) }">{{ orwell?.name }}</span>
      </p>
    ...
    <section class="lab-section">
      ...
      <p>
        ...
        <span>{{ bookstores.totalStores }}</span>
      </p>
      ...
      <p>Our #1 seller: {{ bookstores.topSellers.join(',') }}</p>
    </section>
...
const isHighlighted = (authorName) => {
  return authorName === 'George Orwell';
}
</script>
